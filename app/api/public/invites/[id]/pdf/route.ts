...
export async function GET(
  ...
  const displayName = fullName ? `Nom : ${fullName}` : "Nom : ………………………………";
  ...
  const eventTitle =
    eventData?.name || invite.eventName || invite.eventTitle || "Évènement à confirmer";
  const eventDate = eventData?.date || invite.eventDate || "Date à confirmer";
  const eventTime =
    eventData?.time ||
    invite.eventTime ||
    "Accueil et programme précisés ultérieurement";
